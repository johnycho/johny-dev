...
//   - POST /api/view-count  { id }                  → 해당 글 조회수 +1, 최신값 반환
//   - GET  /api/view-count?ids=/blog/a,/blog/b       → 여러 글 조회수 일괄 조회
//
...
// Upstash 연결 시 환경변수에 접두어가 붙을 수 있어 접미어로 자동 탐지한다.
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const vals = await redis(['MGET', ...ids.map(keyOf)]);
      const counts = {};
      ids.forEach((id, i) => {
