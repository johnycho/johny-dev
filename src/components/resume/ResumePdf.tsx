...
const s = StyleSheet.create({
  ...
  headRow: {flexDirection: 'row', alignItems: 'baseline', marginBottom: 14},
  ...
  h2: {
    ...
    marginTop: 9, marginBottom: 5, paddingBottom: 3,
  ...
  osRow: {flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', marginBottom: 5},
          ...
          {OPENSOURCE.map((o) => (
            <View key={o.prLabel} style={{marginBottom: 9}} wrap={false}>
              <View style={s.osRow}>
                ...
                <View style={{marginLeft: 4, marginTop: 2}}><IcExtSmall /></View>
              </View>
...
export function ResumeDownloadButton({className}: {className?: string}) {
  ...
  return (
    <button type="button" className={className} onClick={onClick} disabled={loading}>
      {loading ? <Spinner /> : <DownloadIcon />} 이력서 PDF
    </button>
  );
}
