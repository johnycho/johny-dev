// 게시글 조회수 설정 + 클라이언트 헬퍼
//
// 백엔드: tools/johny-utils 의 api/view-count.js (Upstash Redis 카운터).
// VIEW_COUNT_API 가 비어 있으면 조회수 UI 는 조용히 렌더링되지 않습니다.

export const VIEW_COUNT_API = 'https://johny-utils.vercel.app/api/view-count';
...
export async function recordView(id: string): Promise<number | null> {
  ...
  try {
    alreadyToday = typeof localStorage !== 'undefined' && localStorage.getItem(`jd:viewed:${key}`) === yyyymmdd();
  } catch (_) {}
  ...
  try {
    ...
    if (j && j.ok) {
      try {
        localStorage.setItem(`jd:viewed:${key}`, yyyymmdd());
      } catch (_) {}
