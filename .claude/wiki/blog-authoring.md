...
- **문장·표현 차원의 스타일은 [phrasing-style.md](./phrasing-style.md), 개별 단어 순화는 [word-choice.md](./word-choice.md)를 따른다.** phrasing-style: **문체(존댓말 통일)·톤(담백)** · 모호한 정성 표현 구체화 · 모순처럼 읽히는 진술의 층위 명시 · 문장 조각·명사화 금지 · 섹션 제목 구체화 · 근거 링크 정밀도(사실 vs 도출) · 보충 노트 형식·위치 등. **두 파일 모두 작성·수정·리뷰 시 목록 전체를 대조**한다(각 파일의 "검사 방법"). 새 규칙은 해당 파일 표에 한 행 추가.
...
- **지양어 스캔은 [word-choice.md](./word-choice.md) 표의 '지양' 좌열 전체를 대조한다** — 즉석에서 떠오르는 일부 단어만 grep하면 최근 추가된 항목이 누락된다. 좌열을 통째로 묶어 한 번에 스캔할 것(상세는 word-choice.md "검사 방법").
