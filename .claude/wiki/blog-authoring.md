...
- **판단 과정을 독자가 따라오게 쓴다.** ① 여러 후보 중 하나를 고른 글이면, 각 후보를 기준에 하나씩 대입해 "어디서 왜 탈락하는지"를 단계별로 보여주고 요약 표는 그 뒤에 둔다. ② 앞 섹션을 번호로 참조할 때("제약 2의 규칙")는 그 내용을 괄호로 한 번 더 적어(`제약 2의 규칙(한 번에 읽는 개수 = 커밋 간격)`) 독자가 되짚지 않게 한다. ③ 설명 뒤 자연히 떠오를 반론·의문(예: "그 옵션만 켜면 되지 않나?")을 예상해 바로 짚어 답한다. ④ 규칙·트레이드오프·타임라인은 말로만 두지 말고 **예시 코드나 Mermaid 시퀀스 다이어그램**으로 보여준다(ASCII 타임라인 대신 `sequenceDiagram`).
