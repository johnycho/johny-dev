...
- **인용·근거는 주장을 정확히 뒷받침하는 것만 쓴다.** 어떤 주장(예: "커서는 별도 커넥션을 쓴다")에 인용을 붙일 때, 그 인용문이 그 주장을 **직접** 뒷받침하는지 확인한다. 공식 문서에 그 내용이 없으면 **추론을 인용처럼 포장하지 말고**, 프레임워크·라이브러리 **소스 코드에서 근거를 찾아** 코드 인용(핵심 몇 줄만 발췌 + `...`로 생략 + 파일 링크)으로 제시한다. 인용·코드 바로 아래에는 **한 줄 해석(`→ …`)** 을 붙여 그게 무엇을 뜻하는지 풀어 준다. 주장마다 근거를 따로 붙이고, 여러 인용을 뭉뚱그리지 않는다. (짧은 발췌만, 저작권 주의)
- **판단 과정을 독자가 따라오게 쓴다.** ① 여러 후보 중 하나를 고른 글이면, 각 후보를 기준에 하나씩 대입해 "어디서 왜 탈락하는지"를 단계별로 보여주고 요약 표는 그 뒤에 둔다. ② 앞 섹션을 번호로 참조할 때("제약 2의 규칙")는 그 내용을 괄호로 한 번 더 적어(`제약 2의 규칙(한 번에 읽는 개수 = 커밋 간격)`) 독자가 되짚지 않게 한다. ③ 설명 뒤 자연히 떠오를 반론·의문(예: "그 옵션만 켜면 되지 않나?")을 예상해 바로 짚어 답한다. ④ 규칙·트레이드오프·타임라인은 말로만 두지 말고 **예시 코드나 Mermaid 시퀀스 다이어그램**으로 보여준다(ASCII 타임라인 대신 `sequenceDiagram`). ⑤ 섹션 제목은 모호한 수식("얇은/최소한") 대신 그 대상이 하는 일을 담는다("버퍼 역할만 하는 …").
