...
- **`**볼드**`가 문장부호로 끝나고 뒤에 한글이 붙으면 렌더되지 않는다** — 닫는 `**` 바로 앞이 `)`·`.` 등 문장부호이고 그 다음이 공백 없이 한글(조사)이면(예: `**복제본(read replica)**을`), CommonMark 규칙상 강조가 닫히지 않아 `**`가 그대로 보인다. **영문 글로스·괄호는 볼드 밖으로 빼서 볼드가 글자로 끝나게 한다**: `**읽기 전용 복제본**(read replica)을`. (같은 이유로 `**무상태**(stateless)여야`처럼 쓴다.)
