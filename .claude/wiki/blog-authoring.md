...
- **`**볼드**`가 닫는 괄호·따옴표로 끝나고 뒤에 한글이 공백 없이 붙으면 렌더되지 않는다** — 닫는 `**` 바로 앞이 `)`·`]`·`'`(닫는 따옴표) 등이고 그 다음이 공백 없이 한글(조사)이면(예: `**복제본(read replica)**을`), CommonMark 규칙상 강조가 닫히지 않아 `**`가 그대로 보인다.
  - **해결: 괄호는 그대로 볼드 안에 두고, 닫는 `**` 뒤에 한 칸 띄운다** — 볼드 뒤에 공백이 오면 강조가 정상적으로 닫혀 적용된다. 예: `**복제본(read replica)** 을`, `**무상태(stateless)** 여야`. (볼드 밖으로 글로스를 빼내지 말 것 — 괄호까지 볼드에 포함되게 유지한다.)
