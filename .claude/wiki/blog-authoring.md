...
- **톤: 담백한 기술 문장.** 과장·극적 비유를 남발하지 않는다. 특히 **범인·용의자·지배자·우두머리·함정·정체** 같은 수사적·AI스러운 표현은 피하고 **원인·문제 객체·가장 많이 차지하는 것·주의할 점** 등 담백한 말로 쓴다. 비유는 이해를 돕는 선에서 절제해서만.
  - **구어적·비유적 표현 대신 일반적인 기술 용어를 쓴다.** 예: 빡빡한→엄격한, 나눠 갚기→(비용) 분산·줄이기, 짊어지다→맡다·담당하다, 노리다→높이다·우선하다, 내주다→낮아지다, 좀→조금·다소, 열쇠→핵심, 한 몸이 되다→하나로 묶이다, 양날→장단·주의할 점, 딴 살림→따로 존재, 걷어내다→제거하다, 쏟아지다→몰리다, 살리다→지키다, 알아채다→감지하다·확인하다, 넘치다→초과하다·가득 차다, 죽이다→강제 종료하다, 죽다→종료되다·중단되다·장애가 나다, 서버가 뜨다→기동하다, 무너지다→문제가 생기다·나빠지다, 미리 구워→미리 만들어.
  - 문맥상 자연스러운 기술 표현은 허용한다: **기동**(시작보다 적합할 때), **덤프를 뜨다**, **정합성·파싱·안전장치가 깨지다**, **락이 풀리다**, **훌륭하다**. 억지로 바꾸지 않는다.
  - 단, 개념 이해를 돕는 **일관된 비유**(예: GC=세차, 메모리=집/방, LLM=두뇌·손발)는 허용한다 — 산발적·구어적 표현만 지양.
