...
문제의 값 객체(`OrderItem`)에는 `equals`/`hashCode`가 없었습니다. 그러면 비교가 `Object.equals`, 즉 **참조(주소) 비교**로 처리됩니다. 6.2 초기 버전은 스냅샷이 원본과 참조를 공유해 **우연히** 통과했고, 7.x는 매번 다른 인스턴스라 통과할 수 없습니다.
