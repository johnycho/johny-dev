...
[Spring Boot 4로 올리는 작업](/blog/spring-boot-4-highlights) 중, 특정 엔티티에서 **읽기만 했는데 커밋 시점에 UPDATE가 나가고 `version`이 1씩 오르는** 현상이 생겼습니다. 낙관적 락 충돌 알람이 분당 여러 건씩 올라왔죠. 코드는 엔티티를 조회해 상태만 판정할 뿐 어떤 필드도 대입하지 않는데도요. 범인은 **`@JdbcTypeCode(SqlTypes.JSON)` 컬럼의 dirty check 방식이 Hibernate 6.2 → 7.x에서 바뀐 것**이었습니다.

<mark>Hibernate 7.x는 JSON 컬럼의 스냅샷을 "직렬화 → 역직렬화"로 deep copy해서 뜬다 — 값 객체에 `equals`가 없으면 참조 비교로 떨어져, 조회만 해도 매 flush마다 dirty로 판정된다.</mark>
...
표면적으로는 "다른 트랜잭션과의 버전 경합"이지만, 경합의 한쪽이 **존재하지 않아야 할 UPDATE**였다는 점이 핵심입니다. 전환 이후 `version` 평균 증가량도 눈에 띄게 늘었습니다.
...
문제의 값 객체(`OrderItem`)에는 `equals`/`hashCode`가 없었습니다. 비교는 `Object.equals`, 즉 **참조 비교**로 떨어집니다. 6.2 초기 버전은 스냅샷이 원본과 참조를 공유해 **우연히** 통과했고, 7.x는 매번 다른 인스턴스라 통과할 수 없습니다.

<mark>결함(값 객체에 `equals` 부재)은 원래 있었고, 버전업이 그것을 드러냈을 뿐이다 — 6.2 초기 버전이 참조를 공유하며 결함을 가려 주고 있었다.</mark>
...
<mark>단, 전제 조건이 있다 — 이 값을 절대 제자리에서 수정하지 않고 항상 통째로 새 리스트/객체로 교체해야 한다. 이 규약을 어기면 다시 "변경 감지 못 함" 버그로 돌아간다.</mark>

### ② 값 객체에 `equals`/`hashCode`

제자리 수정을 허용해야 한다면, JSON에 담기는 값 객체에 `equals`/`hashCode`를 정의합니다. 스냅샷이 새 인스턴스여도 값이 같으면 동일로 판정되어 유령 UPDATE가 사라집니다.
...
- **읽기 경로는 `@Transactional(readOnly = true)`로.** 읽기만 하는 메서드가 쓰기 트랜잭션에 묶여 있으면 flush가 일어나 이 부류의 문제에 노출됩니다. readOnly면 flush 자체가 없어 유령 UPDATE가 원천 차단됩니다. (이력 INSERT 하나 때문에 클래스 전체 `@Transactional`이 풀려 있었다면, 그 메서드만 분리하는 게 낫습니다.)
...
<mark>증상으로 찾는 법 — 읽기만 하는 API를 호출한 뒤 `version`이 올라가거나, 특정 JSON 컬럼만 SET하는 UPDATE가 반복되면 같은 건이다. 전환 전후로 `version` 평균 증가량을 비교하면 빠르게 드러난다.</mark>

## 곁가지 — 같은 전환에서 나온 다른 JSON 이슈

원인은 다르지만 걸리는 자리가 같은(`@JdbcTypeCode(JSON)` 컬럼) 건이 하나 더 있습니다. **Spring Boot 4는 애플리케이션 JSON 라이브러리를 Jackson 3로 바꾸지만, Hibernate의 JSON 컬럼 매퍼는 여전히 Jackson 2 기반**입니다. 이 과정에서 `spring-boot-starter-web`이 `jackson-datatype-jsr310`을 더 이상 전이(transitive) 의존성으로 넘겨주지 않는 경우가 있어, JSON 컬럼 안에 `LocalDate`/`LocalDateTime`이 있으면 `Java 8 date/time type not supported`로 실패할 수 있습니다.
...
- Hibernate가 JSON 컬럼 스냅샷을 **deep copy(직렬화 왕복)** 로 뜨도록 바뀌면서, 값 객체에 `equals`가 없으면 **조회만 해도 dirty**로 판정된다.
- 이는 원래 있던 "in-place 변경 미감지" 버그([HHH-16682])를 고치며 생긴 부작용([HHH-18971])이다. 결함(값 객체 `equals` 부재)은 전부터 있었고 버전업이 드러냈을 뿐이다.
- 해결: **① `@Mutability(Immutability.class)`**(항상 새 참조로 교체할 때·deep copy 자체를 끔) 또는 **② 값 객체에 `equals`/`hashCode`**. 읽기 경로 `readOnly`, 낙관적 락 재시도를 함께 손본다.
- 6.2 후기·6.3·6.4·7.x로 올라가는 서비스는 **JSON 컬럼 전수 조사 + 회귀 테스트 한 줄**로 선제 점검할 수 있다.
