...
<mark>Hibernate 7.x는 flush마다 JSON 컬럼 값을 **직렬화했다가 다시 역직렬화해** 스냅샷을 만든다(deep copy). 이때 값 객체에 `equals`가 없으면 **값이 같아도 서로 다른 객체로 취급**되어, 조회만 했는데도 매번 변경(dirty)으로 판정된다.</mark>
