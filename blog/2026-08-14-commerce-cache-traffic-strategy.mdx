...
상품이 수백만 개인 커머스에서 **모든 상품을 캐시에 올려두는 건 불가능**합니다(메모리도 비용도 안 됩니다). 그래서 보통은 "요청이 오는 상품만 캐시에 담고, 안 쓰는 건 퇴출하는" 방식으로 운영합니다. 문제는 <mark>캐시에 없는 상품에 갑자기 트래픽이 몰릴 때 — 그 부하가 그대로 DB로 몰린다는 것</mark>입니다. 평소 조용하다 입소문으로 인기를 얻은 상품, 방금 나온 신상품처럼요. 이런 상황을 케이스별로 나눠 "부하가 DB로 쏠리지 않게" 대응하는 방법을 정리합니다. (Redis 운영 일반 이슈는 [대용량 Redis 대응](./redis-large-scale-pitfalls) 글을 참고하세요.)

## 전제 — "전량 캐싱"이 아니라 "요청 오는 것만"

상품 전체를 저장할 수는 없으니, 실제로는 이렇게 씁니다.

- **요청 시 채우기(lazy/read-through)**: 캐시에 없으면 DB에서 읽어 캐시에 넣고 응답. 자주 조회되는 상품만 자연히 캐시에 남습니다.
- **퇴출(eviction)**: 메모리가 차면 `allkeys-lfu`(자주 안 쓰는 것부터) 정책으로 정리.

Spring Boot(Spring Data Redis)라면 이 read-through는 `@Cacheable` 한 줄로 표현됩니다 — 캐시에 없을 때만 메서드가 실행되고 결과가 캐시에 저장됩니다.
...
![PlantUML 캐시 히트·미스에 따른 DB 부하 흐름](/img/c4/pu-cache-decision.svg)
...
**대응**:
- **로컬(인메모리) 캐시**를 어플리케이션 서버 앞단에 둡니다 — TTL 몇 초짜리라도, 서버당 그 상품 조회를 대부분 흡수합니다.
...
> Caffeine의 `build(loader)`는 같은 키에 동시 요청이 몰려도 로더를 **한 번만** 호출합니다 — 이게 요청 병합(single-flight)입니다.

<AnimatedLineChart
  title="핫키 급등 — 유입은 급증해도 DB 도달은 낮게 유지"
...
**대응**:
- **프리워밍(pre-warming)**: 노출 전에 캐시를 미리 채웁니다. 신상품 등록/기획전 오픈 직전에 배치로 넣어두면 콜드 스타트 자체가 사라집니다.
...
- 그래도 초과하면 **대기열/로드 셰딩**으로 유입을 고릅니다(아래).

## 마지막 방어선 — 로드 셰딩·서킷 브레이커

위 대응을 다 해도 DB 용량을 넘는 순간이 옵니다. 그땐 **전부 받다 전부 멈추기**보다 일부를 포기해 전체를 지킵니다.

- **로드 셰딩(load shedding)**: DB 부하가 임계치를 넘으면 초과 요청을 빠르게 거절(`429 Too Many Requests`)하거나 기본 화면을 줍니다.
...
| 한계 초과 | DB 장애 위험 | 로드 셰딩, 서킷 브레이커 |

<mark>전체 구조는 3중 방어입니다 — ①앞단에서 흡수(로컬 캐시·요청 병합), ②DB로 가는 건 한 번만(재생성 잠금·프리워밍), ③초과하면 일부를 포기(로드 셰딩).</mark> 전량 캐싱이 불가능한 규모에서는 "캐시에 없을 때 어떻게 행동하느냐"를 설계해 두는 것이 트래픽 대응입니다.
