...
>
> `n`은 **구독자(Subscriber)** 가 정합니다. 구독자가 `onSubscribe(Subscription s)`에서 받은 `Subscription`에 `s.request(n)`을 호출할 때 넘기는 숫자입니다(이때 파라미터 `s`가 곧 그 `Subscription` 객체입니다). 직접 구현한 구독자면 개발자가 코드에 명시하고(아래 예시), Reactor·WebFlux처럼 프레임워크가 구독을 대행하면 그 연산자·최종 구독자가 정합니다 — 기본값은 대개 `Long.MAX_VALUE`(무제한)이고, `limitRate(n)` 같은 연산자로 상한을 둡니다. WebFlux HTTP 스트리밍에서는 응답을 내보내는 쪽이 **전송 계층(TCP 소켓)의 쓰기 여유**에 맞춰 `request`를 내, 소켓이 막히면 요청도 멈춥니다.

직접 구현한 구독자에서 `n`을 정하는 모습은 이렇습니다.

```java
// 직접 구현한 구독자 — n을 개발자가 코드에 명시한다
...
반면 Reactor·WebFlux처럼 프레임워크가 구독을 대행하면 이 `n`을 대신 정해 줍니다. 이건 추정이 아니라 [Reactor 레퍼런스 가이드](https://projectreactor.io/docs/core/release/reference/reactiveProgramming.html#reactive.backpressure)에 명시돼 있습니다 — 가장 직접적인 구독 방식(`subscribe()`·람다형 변형)은 구독 시 곧바로 **무제한(`Long.MAX_VALUE`)을 요청**하고("the most direct ways of subscribing all immediately trigger an unbounded request of `Long.MAX_VALUE`"), 상한이 필요하면 [`limitRate(n)`](https://projectreactor.io/docs/core/release/api/reactor/core/publisher/Flux.html#limitRate%28int%29) 연산자가 하위 수요를 **`n`개 단위 배치로 쪼개** 위로 전파합니다.

## 발행과 구독이 이어지는 구조

실제 코드는 **소스 → 연산자 여러 단계 → 최종 구독자**가 여러 단계가 이어진 **파이프라인**입니다. 방향이 두 갈래인 게 핵심입니다.
...
- [`Flux.limitRate(int)` 자바독](https://projectreactor.io/docs/core/release/api/reactor/core/publisher/Flux.html#limitRate%28int%29) — 하위 수요를 배치로 쪼개 상한
