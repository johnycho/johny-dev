...
문제가 되는 조합은 **gap 락 + INSERT**입니다. gap 락끼리는 서로 **공존**할 수 있지만, `INSERT`가 잡는 **insert intention 락**은 다른 트랜잭션이 보유한 gap 락과 **충돌해 대기**합니다.
...
- 스프링 `@Transactional`의 기본은 **DB 기본값**을 따르므로, 격리에 의존하는 로직은 **수준을 명시**합니다.
- 격리 수준으로 **읽기 일관성**을 잡고, **동시 갱신 경합(lost update)** 은 잠금으로 따로 막습니다.
