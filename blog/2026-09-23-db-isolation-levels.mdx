...
non-repeatable read가 **한 행의 값**이 바뀌는 문제라면, phantom read는 **조건에 맞는 행의 집합**이 바뀌는(새 행이 끼어드는) 문제입니다.
...
여기서 **핵심 오해**가 하나 있습니다. 같은 이름의 격리 수준이라도 **DB마다 실제 막아 주는 범위가 다릅니다.**

- **MySQL (InnoDB)**
  - 기본: `REPEATABLE READ`
  - `REPEATABLE READ`에서 **next-key 락(gap 락)** 으로 새 행이 끼어드는 것까지 막아 **phantom read도 차단**합니다(표준 초과).
...
<mark>MySQL InnoDB에서는 그렇습니다. REPEATABLE READ가 phantom을 막으려고 쓰는 **gap(next-key) 락**이 잠금 대기·데드락을 늘립니다.</mark> RR은 "행"만이 아니라 **"그 행 앞의 빈 구간(gap)"** 까지 잠가 새 행이 끼어드는 것을 막는데, 구간을 잠그다 보니 충돌 범위가 넓어집니다.
...
문제가 되는 조합은 **gap 락 + INSERT**입니다. gap 락끼리는 서로 **공존**할 수 있지만, `INSERT`가 잡는 **insert intention 락**은 남이 잡은 gap 락과 **충돌해 대기**합니다.
...
-- B가 잡은 gap 락 때문에 대기                    -- A가 잡은 gap 락 때문에 대기 → 데드락!
```
