...
서버가 한 대일 때는 자바 `synchronized`나 `ReentrantLock`으로 동시성을 막습니다. 그런데 <mark>서버를 여러 대로 늘리는 순간, 언어 차원의 락은 "그 프로세스 안에서만" 통해서 무력해집니다.</mark> 그래서 여러 인스턴스가 공유하는 **분산락**이 등장합니다. 이 글은 분산락이 왜·언제 꼭 필요한지, 그리고 그 락을 올린 Redis에 장애가 나도 서비스가 멈추지 않게 하려면 무엇을 해야 하는지 정리합니다. (Redisson 구현·워치독은 [Redis 분산락](./redis-distributed-lock) 글 참고)
...
Container(s1, "인스턴스 1", "어플리케이션 서버")
Container(s2, "인스턴스 2", "어플리케이션 서버")
Container(s3, "인스턴스 3", "어플리케이션 서버")
ContainerDb(redis, "공유 락", "Redis 등 외부 저장소", "한 번에 하나만 임계 구역 진입")

Rel(s1, redis, "락 획득 시도")
Rel(s2, redis, "락 획득 시도")
Rel(s3, redis, "락 획득 시도")
@enduml
```

</details>

![C4 여러 인스턴스가 공유하는 Redis 락](/img/c4/redis-lock-1.svg)
...
- **여러 인스턴스에 걸린 스케줄러·배치의 중복 실행 방지** — 서버 3대에 같은 크론(`@Scheduled`)이 붙어 있으면 "매일 새벽 정산 배치"가 3번 실행됩니다. 분산락으로 **그 시각에 한 대만** 실행하게 합니다. 예: 정산 배치, 만료 포인트·쿠폰 일괄 회수, 리포트 생성, 대량 푸시 발송. (이 용도의 라이브러리로 ShedLock이 있습니다.)
...
Container(ca, "클라이언트 A", "어플리케이션")
Container(cb, "클라이언트 B", "어플리케이션")
ContainerDb(master, "마스터", "Redis", "복제 전 다운 → 마스터 장애")
ContainerDb(replica, "복제본 → 새 마스터", "Redis", "락 기록 없음")

Rel(ca, master, "락 획득")
Rel(master, replica, "복제 (비동기)")
Rel_Up(replica, master, "페일오버로 승격")
Rel(cb, replica, "같은 락 획득 성공")
Rel(cb, ca, "A·B 동시 진입 → 상호배제 위반", "충돌")
@enduml
```

</details>

![C4 Redis 마스터·복제 페일오버 시 락 이중 획득](/img/c4/redis-lock-2.svg)

import ArchifyEmbed from '@site/src/components/ArchifyEmbed';

<ArchifyEmbed src="/diagrams/distributed-lock-failover.html" title="Redis 페일오버 시 락 이중 획득 (인터랙티브)" height="520px" />
...
핵심은 하나입니다 — <mark>"락이 잠깐 풀려도 데이터는 안전"하도록, 정합성의 최종 방어선을 DB에 둡니다.</mark>
...
- 서버가 여러 대면 언어 락이 안 통한다 → **분산락**이 여러 노드가 공유하는 락을 제공합니다.
- 그러나 **재고·중복 생성·동시 수정은 대개 DB(원자 연산·유니크·낙관/비관 락)로** 풀립니다. 분산락은 스케줄러 중복 방지·외부 자원처럼 DB로 어려운 곳에.
- Redis는 SPOF이고, **비동기 복제 탓에 페일오버 시 락이 이중 획득될 수 있습니다.**
- 그래서 <mark>분산락은 "최적화", 정합성의 최종 보루는 DB에 두고, 펜싱 토큰·락 실패 처리·HA로 장애를 견디게 설계합니다.</mark>
