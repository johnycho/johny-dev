...
:외부 API 느려짐;
:요청마다 최대 30초 대기;
:Tomcat 워커 스레드 + DB 커넥션을 30초씩 점유;
:피크에 요청이 쌓이며 스레드·커넥션 풀 고갈;
:새 요청을 못 받음 → 서버 전체 Hang;
:헬스체크 실패 → K8s가 Pod 재시작;
stop
@enduml
```
...
![PlantUML 느린 외부 API로 인한 서버 행 연쇄](/img/c4/pu-api-hang.svg)
...
- **벌크헤드(격리)** — 외부 호출용 스레드 풀을 따로 둬, 그게 막혀도 나머지 요청은 지킵니다.
- **타임아웃 예산** — 외부일수록 길게·안쪽일수록 짧게 차등. 재시도는 백오프+상한.
...
<mark>외부 의존은 "언젠가 느려진다"를 전제로, 빨리 실패하고 번지지 않게 격리하도록 설계해야 합니다.</mark>
