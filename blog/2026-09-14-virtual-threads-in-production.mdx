...
가상 스레드는 **OS 스레드를 더 만들어 주지 않습니다.** 실제 병렬 연산 능력은 CPU 코어 수에 묶여 있습니다. 이미지 변환·암호화·대량 계산처럼 **CPU를 계속 소모하는** 작업은 블로킹으로 캐리어를 반납할 일이 없어, 가상 스레드로 바꿔도 빨라지지 않습니다. 이런 일은 기존 [스레드 풀](./spring-boot-thread-pools)로 코어 수에 맞춰 제한하는 게 맞습니다.
