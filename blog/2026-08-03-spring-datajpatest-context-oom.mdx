...
테스트를 **하나씩** 돌리면 다 통과하는데, **전부 한꺼번에** 돌리면 메모리 부족(OOM)으로 실패했습니다. 원인은 코드 버그가 아니라 <mark>Spring이 테스트용 "세트(컨텍스트)"를 재사용하지 못하고 18개나 새로 만들어 쌓아둔 것</mark>이었습니다. 흩어진 설정을 **공통 부모로 모아** 18개로 갈려 있던 세트를 5개로 줄이니, 메모리 문제가 사라졌습니다.
