...
- 참고: 예전(**JDK 7 업데이트 4**, 즉 `7u4`·2012년 이전)엔 **Young만 병렬**이고 Old는 단일 스레드였지만, **`7u4`부터 `-XX:+UseParallelGC`가 `UseParallelOldGC`를 함께 켜서** Young·Old를 모두 병렬로 수거합니다(Young만 병렬로 두려면 `-XX:-UseParallelOldGC`).
