...
description: "공유락(S)·배타락(X)의 개념과 호환성, 그리고 READ COMMITTED·REPEATABLE READ·SERIALIZABLE 격리 수준별로 언제 락을 잡고 대기하거나 데드락이 발생하는지를 MySQL InnoDB 기준 예시·다이어그램으로 정리합니다."
---
...
- **공유락(S, shared)** — **읽기**용. 여러 트랜잭션이 같은 행에 **동시에 S락을 잡을 수 있습니다**(읽기끼리는 서로 방해 안 함). `SELECT ... FOR SHARE`(구 `LOCK IN SHARE MODE`).
- **배타락(X, exclusive)** — **수정·삭제**용. **한 트랜잭션만** 잡을 수 있고, 다른 트랜잭션은 S든 X든 잡지 못합니다. `UPDATE`·`DELETE`·`INSERT`, `SELECT ... FOR UPDATE`.
...
<mark>S–S만 공존하고, X가 끼면 무조건 대기입니다.</mark> 충돌하는 락을 남이 잡고 있으면 풀릴 때까지 기다리는데(lock wait), 기본 대기 한도는 `innodb_lock_wait_timeout`(기본 50초)이고 넘으면 오류가 납니다.
...
A -> R : 행1 X락 획득
B -> R : 행2 X락 획득
A -> R : 행2 X락 요청 → B가 잡음 → 대기
B -> R : 행1 X락 요청 → A가 잡음 → 대기
...
- **트랜잭션을 짧게** — 락을 잡은 시간이 짧을수록 충돌이 줄어듭니다.
