...
<mark>S–S만 공존하고, X가 끼면 무조건 대기입니다.</mark> 충돌하는 락을 다른 트랜잭션이 보유하고 있으면 해제될 때까지 기다리는데(lock wait), 기본 대기 한도는 `innodb_lock_wait_timeout`(기본 50초)이고 넘으면 오류가 납니다.

### S락은 왜 필요할까?

일반 `SELECT`는 MVCC 스냅샷을 락 없이 읽으니 보통 S락이 필요 없습니다. 하지만 <mark>**읽은 값이 커밋까지 다른 트랜잭션에 의해 바뀌지 않도록 보장**해야 할 때</mark>가 있습니다 — 예를 들어 재고를 읽고 그 값을 근거로 주문을 만들 때, 읽는 순간부터 커밋까지 그 행을 다른 트랜잭션이 못 바꾸게 해야 안전합니다.
...
- **대기(lock wait)** — 한 방향으로 기다리는 상태. 상대가 커밋·롤백해 락을 해제하면 이어서 진행합니다. 대기가 너무 길어지면 타임아웃 오류가 납니다.
- **데드락(deadlock)** — 둘 이상이 **서로 상대의 락을 기다리는 순환**. 아무도 해소하지 못해 InnoDB가 감지해 **한쪽을 롤백**합니다(오류 1213). 롤백된 쪽은 애플리케이션이 다시 실행해야 합니다.
...
InnoDB는 데드락을 **자동으로 감지**해 스스로 해소합니다([공식 문서](https://dev.mysql.com/doc/refman/8.0/en/innodb-deadlocks.html)). 순환에 걸린 트랜잭션 중 <mark>**작업량이 가장 적은 쪽(바꾼 행 수가 적은 쪽)을 victim으로 골라, 그 트랜잭션 전체를 롤백**하고 나머지는 그대로 진행시킵니다</mark> — 둘 다 죽는 게 아닙니다.
...
**대기 초과(lock wait timeout)는 데드락과 다릅니다.** 순환이 아니라 단순히 오래 기다린 경우입니다.
...
- **필요 이상 높은 격리를 피한다** — SERIALIZABLE 남용 금지. 특정 갱신만 보호하려면 격리 수준을 올리기보다 `SELECT ... FOR UPDATE`로 그 행만 잠급니다.
