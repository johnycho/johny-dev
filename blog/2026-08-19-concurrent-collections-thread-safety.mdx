...
`add(e)`는 사실 여러 단계입니다 — 대략 ① 공간이 부족하면 내부 배열을 늘리고(리사이즈) ② `elementData[size] = e` 로 넣고 ③ `size++`. 이 셋은 하나의 원자적 동작이 아니라서, 여러 스레드가 동시에 하면 이렇게 틀어집니다.
...
fork again
  :스레드 B: size=5 읽음;
  :B: elementData[5]=y (x 덮어씀);
  :size++ → 6 (하나는 유실, 7이어야 할 게 6);
end fork
stop
@enduml
```
...
![PlantUML ArrayList 동시 add 레이스로 데이터 유실](/img/c4/pu-arraylist-race.svg)

- **덮어쓰기 손실** — 두 스레드가 같은 `size`를 읽어 같은 칸에 써, 하나가 사라집니다.
- **size 경합** — `size++`가 겹쳐 실제보다 작게 세어집니다(개수가 모자람).
- **리사이즈 레이스** — 늘리는 도중 다른 스레드가 옛 배열을 참조해 `null`이 섞이거나 `ArrayIndexOutOfBoundsException`.
...
`ConcurrentLinkedQueue`는 락 없이(내부적으로 CAS로) 동시 `add`를 안전하게 처리합니다. 여러 스레드가 동시에 넣어도 유실이 없고, 별도 동기화도 필요 없습니다. 다 넣은 뒤 한 번에 읽어 리스트로 옮기면 됩니다.
...
- **`ConcurrentLinkedQueue`로 넣기** — 직접 락을 걸 필요가 없어 넣는 코드가 단순합니다.
...
두 스레드가 거의 동시에 이 셋을 하면 이렇게 틀어집니다 — A와 B가 **둘 다 5를 읽고**, 둘 다 6을 써서, 두 번 증가했는데 결과는 6(한 번이 유실).
...
<mark>"읽고-더하고-쓰기"가 하나의 원자적 동작이 아니라서, 그 사이에 다른 스레드가 개입하면 증가가 사라지는 것</mark>입니다. 그래서 카운터는 이 세 단계를 원자적으로 묶어 주는 `AtomicInteger`(`incrementAndGet`), 고경합이면 `LongAdder`로 해야 합니다.
...
- `ArrayList`·평범한 컬렉션은 단일 스레드용이라, **동시 `add`는 조용한 데이터 손실**을 유발합니다.
- **가장 먼저 "공유를 없애라"** — 결과를 반환해 한 스레드가 모으면 문제가 사라집니다.
- 꼭 공유해야 하면 `ConcurrentLinkedQueue`·`ConcurrentHashMap` 등 **동시성 자료구조**를 씁니다.
- 람다 안 `int[]` 카운터도 비원자적이라, 카운터는 `AtomicInteger`·`LongAdder`로 씁니다.
...
| 원자적 | 중간에 다른 동작이 개입할 수 없는 하나의 동작 |
