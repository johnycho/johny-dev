...
:signUp() 트랜잭션 시작;
:user 저장;
:grantSignupCoupon() — 같은 트랜잭션에 참여;
if (외부 실패로 예외?) then (예)
  :스프링이 트랜잭션을 rollback-only로 표시;
  :외부 try-catch가 예외를 삼킴;
  :정상처럼 커밋 시도;
  :이미 rollback-only → 커밋 거부\nUnexpectedRollbackException + 전체 롤백;
  stop
else (아니오)
  :정상 커밋;
  stop
...
![PlantUML rollback-only 전파 흐름 — 예외·롤백 표시·커밋 거부](/img/c4/pu-rollback-only.svg)

<mark>내부 `@Transactional`이 던진 예외는, 외부에서 잡아 삼켜도 트랜잭션에 남긴 "rollback-only" 표시까지 지우지는 못합니다.</mark> 그래서 커밋 시점에 스프링이 "이 트랜잭션은 롤백만 가능"이라며 거부하고, 저장한 user까지 전부 롤백됩니다.

## 왜 스프링은 이렇게 동작하나

`REQUIRED` 전파에서 내부 메서드는 **새 트랜잭션이 아니라 상위 트랜잭션에 참여**합니다(물리적으로 하나). 이 참여 구간에서 예외가 터지면, 스프링은 트랜잭션을 **rollback-only**로 표시합니다(`globalRollbackOnParticipationFailure` 기본값).
...
**방법 2 — 선택적 작업을 별도 트랜잭션으로(`REQUIRES_NEW`).** 내부를 독립 트랜잭션으로 분리하면, 내부 실패가 외부를 오염시키지 않습니다.
...
- 외부에서 `try-catch`로 삼켜도 표시는 남아, **커밋 때 `UnexpectedRollbackException`** 으로 전체가 롤백된다.
...
| REQUIRED | 상위 트랜잭션이 있으면 거기에 참여(기본 전파) |
