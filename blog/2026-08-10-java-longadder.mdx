...
여러 칸을 훑어 더하는 특성상, 함수 실행 도중 다른 스레드가 어떤 칸을 바꾸면 그 값이 반영될 수도, 안 될 수도 있습니다. 공식 문서도 <mark>"반환값은 원자적 스냅샷이 아니다(NOT an atomic snapshot)"</mark> 라고 못 박습니다.
...
바꿔 말하면, **쓰기(갱신)가 아주 잦고 읽기(합계)는 가끔** 일어나는 워크로드가 `LongAdder` 의 스위트스폿입니다.
