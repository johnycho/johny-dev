...
헥사고날 아키텍처는 **도메인을 가운데 두고, 웹·DB·외부 API 같은 외부를 어댑터로 연결해 어댑터만 바꾸면 교체되게 하는** 구조입니다. 개념을 코드로만 나누면 경계가 모호해지기 쉬워서, 아예 **멀티모듈로 물리적으로 분리**해 도메인이 인프라를 의존하지 못하게 강제한 구성을 정리합니다. 헥사고날 아키텍처는 [앨리스터 코오번(Alistair Cockburn)](https://alistair.cockburn.us/hexagonal-architecture/)이 2005년 제안한 패턴으로, 원래 이름은 **포트와 어댑터(Ports and Adapters)** 입니다.

<mark>헥사고날의 목표는 "도메인은 외부를 모르고, 외부가 도메인의 포트(인터페이스)를 구현한다"입니다.</mark>

## 핵심: 도메인은 가운데, 어댑터는 외부

외부와의 연결을 두 방향으로 나눕니다.
...
└── sub-module/              # 공유 도메인·코어·어댑터
    ├── app-domain/          # 도메인 모델 + 어플리케이션(포트 in/out + 유스케이스 service)
...
여기서 핵심은 **유스케이스와 포트가 `app-domain` 모듈에 있다**는 점입니다. `app-domain` 모듈이 도메인 모델과 어플리케이션(포트·유스케이스)을 함께 담고, `app-core`는 여러 모듈이 함께 쓰는 **기술 글루**(공통 어댑터·설정·보안)입니다. 각 실행 앱은 `app-domain`·`app-core`에 필요한 아웃바운드 어댑터(`app-persistence`, `app-external-api`)를 조합해 만듭니다. **`app-domain`과 `app-core`는 어댑터 모듈을 의존하지 않고**, 반대로 어댑터가 이들을 의존합니다.
...
화살표 방향이 핵심입니다. 유스케이스(`app-domain`의 `application/service`)도 `app-persistence`도 **둘 다 `app-domain`의 포트(인터페이스)를 향합니다.** 그래서 유스케이스가 `app-persistence`를 import하는 일이 없고, 실제 구현체를 주입하는 것은 실행 앱(구성 지점)이 DI로 합니다.
...
class "유스케이스\nPlaceOrderService" as UC
class "영속성 어댑터\nOrderPersistenceAdapter" as ADP
...
![PlantUML 유스케이스가 포트에 의존하고 어댑터가 포트를 구현](/img/c4/pu-hex-port.svg)

유스케이스와 어댑터가 **둘 다 인터페이스(포트)를 향합니다.** 유스케이스는 `app-persistence`를 import하지 않고, 실제 구현체는 실행 앱이 DI로 주입합니다. 그래서 저장소를 JPA에서 다른 것으로 바꿔도 유스케이스·도메인 코드는 그대로입니다.

<mark>실행 앱은 "어떤 어댑터를 끼울지"만 구성할 뿐, 비즈니스 규칙은 app-domain 모듈(모델·유스케이스)에 있습니다.</mark>
...
└── sub-module/
    ...
    ├── app-domain/                 # [모듈] 도메인 모델 + 어플리케이션 (헥사고날 코어)
    │   ├── application/            #   [패키지] 어플리케이션 (컨텍스트별)
    │   │   ├── order/              #     주문 컨텍스트
...
한 가지 구분할 점: **모듈 `app-domain`과 그 안의 `domain/` 패키지는 서로 다른 단위입니다.** 모듈(`app-domain`)은 빌드 단위로 어플리케이션(`application/`)과 도메인 모델(`domain/`)을 함께 담고, 그 아래 `builder`·`enums`·`exception`·`outbox`는 도메인 모델을 뒷받침하는 형제 패키지입니다. 즉 `app-domain`은 **모듈**, 그 안의 `domain/`은 그 모듈의 **도메인 모델 패키지**입니다.
...
![PlantUML 인바운드·아웃바운드 어댑터가 코어를 향하는 의존 방향](/img/c4/pu-hex-adapters.svg)

import ArchifyEmbed from '@site/src/components/ArchifyEmbed';

<ArchifyEmbed src="/diagrams/hexagonal-multimodule.html" title="헥사고날 멀티모듈 의존 방향 (인터랙티브)" height="460px" />

화살표가 **둘 다 안쪽(도메인)을 향한다**는 게 핵심입니다. 인바운드 어댑터는 도메인의 유스케이스를 호출하고, 아웃바운드 어댑터는 도메인의 포트를 구현합니다. 어느 쪽도 도메인이 외부를 의존하지 않으므로, DB를 바꾸거나 외부 API를 교체해도 도메인·유스케이스 코드는 그대로입니다.
...
participant "인바운드 어댑터(web)" as IN
participant "application core" as C
participant "아웃바운드 어댑터(app-persistence)" as OUT
...
![PlantUML place(order) 요청이 in·out 포트를 거쳐 DB까지 이동하는 순서](/img/c4/pu-hex-seq.svg)
...
| ③ 유스케이스 실행 | 어플리케이션(app-domain) | <mark>app-domain</mark>/…/application/order/<mark>service</mark>/PlaceOrderService |
...
<mark>인바운드 어댑터 → in 포트 → 유스케이스 → out 포트 → 아웃바운드 어댑터 → DB 순으로 이어지고, 유스케이스는 포트(인터페이스)만 알고 어댑터 구현은 모릅니다.</mark>
...
- 레이어드: 위에서 아래로 향하는 계층. 인프라가 맨 아래.
- 헥사고날: 도메인이 중심. 웹이든 DB든 외부 API든 전부 "외부의 어댑터"로 동등하게 취급.

<mark>헥사고날은 인프라(웹·DB·외부 API)를 도메인의 "하위 계층"이 아니라 **언제든 교체할 수 있는 외부 요소**로 봅니다 — 그래서 인프라를 바꾸기 쉽고, 도메인만 분리해 단위 테스트하기도 쉽습니다.</mark>
...
- 의존성은 항상 **도메인 안쪽**으로 향하게 해, 인프라를 교체해도 도메인이 흔들리지 않게 합니다.
...
| 어댑터(adapter) | 포트를 구현/호출하는 외부 코드(웹·DB·외부 API) |
