...
- **소유자가 죽으면(crash)** — 워치독이 멈춰 남은 lease(기본 30초)만 지나면 자동 해제됩니다. 즉 프로세스가 완전히 죽는 건 시간 안에 자가 치유되고, **"죽지 않고 멈춘" 경우가 진짜 문제**입니다.
...
응급 복구가 필요하면 `forceUnlock()`이 있지만, 정상 소유자의 작업을 깨뜨릴 수 있어 **최후 수단**입니다.
