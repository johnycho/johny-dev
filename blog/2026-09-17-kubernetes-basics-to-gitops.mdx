...
컨테이너로 서비스를 하나 띄우는 건 어렵지 않습니다. 그런데 그 컨테이너가 수십 개가 되고, 장애가 나면 다시 복구돼야 하고, 무중단으로 새 버전을 올려야 하고, 두 데이터센터에 나눠 기동돼 있어야 한다면 이야기가 달라집니다. 이 "여러 컨테이너를 대신 운영해 주는 도구"가 **쿠버네티스**입니다. 이 글은 쿠버네티스의 기본 개념부터 시작해, 제가 참여한 프로젝트에서 이를 **어떻게 실제 배포·운영에 적용했는지**까지 정리합니다. (클러스터가 내부적으로 어떤 구성 요소로 이루어지는지는 [쿠버네티스 구성 요소](/blog/kubernetes-components-architecture) 글에서 따로 정리했습니다.)
...
컨테이너(도커)는 "어플리케이션 + 실행 환경"을 하나로 묶어, 어디서든 똑같이 실행되게 해 줍니다. 문제는 **운영**입니다. 컨테이너 하나를 손으로 띄우는 건 쉽지만, 실서비스에서는 이런 게 필요합니다.

- 트래픽이 늘면 같은 컨테이너를 **여러 개로 복제**하고, 줄면 다시 줄인다.
- 컨테이너에 장애가 나면 **자동으로 다시 기동한다.**
- 새 버전을 **순단(정지) 없이** 교체한다.
- 여러 서버(노드)에 컨테이너를 **적절히 배치**하고, 한 서버에 장애가 나도 버틴다.
...
- **Pod(파드)** — 쿠버네티스가 다루는 **가장 작은 배포 단위**. 보통 컨테이너 하나(때로 여러 개)를 감싼 껍데기입니다. 파드는 **언제든 중단되고 새로 기동될 수 있는** 일회성 존재로 봅니다.
- **Deployment(디플로이먼트)** — "이 이미지를 **몇 개(레플리카)** 띄워 둬라"를 선언하는 오브젝트. 실제로는 **ReplicaSet**을 통해 파드 개수를 유지합니다. 파드에 하나 장애가 나면 Deployment가 곧바로 새 파드를 채워 **선언한 개수를 지킵니다.**
- **Service(서비스)** — 파드는 중단·재기동되며 IP가 계속 바뀌므로, 그 앞에 **고정된 진입점**을 두고 트래픽을 여러 파드에 **부하 분산**합니다. 외부에서 들어오는 경로는 **Ingress**가 담당합니다.
...
이 오브젝트들은 **Namespace(네임스페이스)** 로 나누어 환경(dev·stg·운영)이나 팀별로 격리합니다. 그리고 이 모든 파드는 실제 서버인 **노드(Node)** 위에서 동작하고, 노드들의 묶음이 **클러스터**입니다.

<mark>파드는 언제든 사라질 수 있는 일회성 단위이고, Deployment가 "선언한 개수"를 유지하며, Service가 그 앞에서 변하지 않는 진입점을 제공합니다.</mark>

Service 타입(ClusterIP·NodePort·LoadBalancer)과 외부 HTTP 라우팅을 맡는 **Ingress**(그리고 Ingress Controller)의 개념·구성은 [쿠버네티스 구성 요소](/blog/kubernetes-components-architecture) 글의 네트워킹 부분에서 정리했습니다.

## 쿠버네티스의 핵심 철학 — 선언형과 원하는 상태

쿠버네티스의 핵심 사고방식이 하나 있습니다. **명령형이 아니라 선언형**이라는 점입니다.
...
그래서 파드에 장애가 나도, 노드가 빠져도, 실수로 하나를 지워도 **선언한 상태로 자동 복구**됩니다. 뒤에서 볼 GitOps·self-heal이 전부 이 원리의 연장입니다.
...
<mark>"어떻게 할지"가 아니라 "무엇을 원하는지"를 적어 두면 시스템이 그 상태를 유지합니다 — 이것이 쿠버네티스 운영이 손이 덜 가는 이유입니다.</mark>

## 매니지드 쿠버네티스(EKS)를 쓴 이유

쿠버네티스는 기능이 많지만, **직접 운영하기는 까다롭습니다.** 클러스터의 제어를 맡는 **컨트롤 플레인**(API 서버·스케줄러·etcd 등)을 직접 설치·이중화·업그레이드·백업해야 하기 때문입니다. 프로젝트에서는 이 부담을 클라우드에 위임하는 **매니지드 쿠버네티스(AWS의 EKS)** 를 선택했습니다.
...
<mark>매니지드 쿠버네티스는 "쿠버네티스를 운영하는 일" 자체를 클라우드에 맡기고, 팀은 어플리케이션 배포·운영에만 집중하게 해 줍니다.</mark>

이 글에서 다루는 AWS 인프라를 한 장으로 보면 이렇습니다. 개발자·CI가 이미지를 **ECR(Elastic Container Registry, 컨테이너 이미지 저장소)** 에 올리고 매니페스트를 **Git**에 커밋하면, **EKS(Elastic Kubernetes Service, 매니지드 쿠버네티스)** 안의 ArgoCD가 이를 감지해 **워커 노드(EC2, Elastic Compute Cloud 가상 서버)** 에 배포하고, 노드는 ECR에서 이미지를, **Secrets Manager**에서 시크릿을 가져옵니다. 각 요소는 이어지는 절에서 자세히 다룹니다.

<details>
<summary>PlantUML 코드 (AWS 아이콘)</summary>
...
title EKS·GitOps 인프라 관계도

User(dev, "개발자 / CI", "")
rectangle "매니페스트 저장소 (Git)" as git

AWSCloudGroup(aws){
  ElasticContainerRegistry(ecr, "ECR", "컨테이너 이미지")
  SecretsManager(sm, "Secrets Manager", "앱 시크릿")
  ElasticKubernetesService(eks, "EKS 클러스터", "ArgoCD 포함")
  EC2(nodea, "Worker Node · 가용영역 A", "Pod x2")
  EC2(nodec, "Worker Node · 가용영역 C", "Pod x1")
}

dev --> ecr : 이미지 push
dev --> git : 매니페스트 commit
eks --> git : ArgoCD가 변경 감지
eks --> nodea : 배포
eks --> nodec : 배포
nodea --> ecr : 이미지 pull
nodea --> sm : 시크릿 로드
...
![PlantUML EKS·GitOps AWS 인프라 관계도](/img/c4/k8s-aws.svg)
...
- **Git이 배포 이력**입니다. 언제·누가·무엇을 바꿨는지 커밋 로그에 남고, 되돌리기는 커밋 되돌리기입니다.
...
그리고 배포 파이프라인이 남기는 커밋의 실제 내용은 대개 **이미지 태그 한 줄**입니다. 이 커밋이 "배포"입니다.
...
<mark>GitOps에서는 클러스터에 직접 명령하지 않고 "원하는 상태를 Git에 적어 두면" ArgoCD가 맞춰 줍니다 — 배포가 커밋이고, 롤백이 되돌리기입니다.</mark>
...
- **블루-그린(blue-green)**: 새 버전을 완전히 기동해 두고, 준비되면 트래픽을 **한 번에 전환**합니다. 문제가 생기면 예전 버전으로 바로 되돌립니다.
...
블루-그린은 `strategy.blueGreen`으로 선언합니다. 새 버전을 **preview 서비스**로 미리 기동해 두고, 승인하면 트래픽을 **active 서비스**로 한 번에 전환합니다.
...
한 데이터센터(가용영역, AZ)가 통째로 내려가도 서비스가 유지돼야 합니다. 그래서 워크로드를 **두 개의 가용영역에 나눠** 배치합니다. 한 AZ에 장애가 나도 다른 AZ의 레플리카가 트래픽을 받습니다.
...
주기적으로 실행되는 배치 작업은 쿠버네티스의 **CronJob**으로 실행합니다. 정해진 스케줄에 파드를 하나 기동해 작업을 수행하고 끝나면 사라집니다. 배치에서 가장 조심할 것은 **중복 실행**입니다.
...
이 옵션들을 반영한 CronJob 매니페스트는 이렇습니다.
...
- **어플리케이션이 클라우드 시크릿 매니저에서 직접 로드**합니다. 매니페스트에는 "어디서 읽어라"만 있고 값은 없습니다.
...
- **개념**: 파드는 일회성 단위, Deployment가 개수를 유지, Service가 고정 진입점입니다. 모든 게 **선언형(원하는 상태)** 위에서 동작합니다.
...
- **가용성**: **멀티 AZ + self-heal + 무중단 배포 + 오토스케일**이 겹쳐 만듭니다.
- **배치**: **CronJob** — `Forbid`와 컷오버 순서로 이중 실행 방지.
- **보안**: 시크릿은 매니페스트 밖(시크릿 매니저), 권한은 **Pod Identity 최소 부여**.

쿠버네티스를 처음 볼 때는 오브젝트 종류가 많아 복잡해 보이지만, **"원하는 상태를 선언하면 시스템이 맞춘다"** 는 원리 하나만 이해하면 GitOps도, self-heal도, 롤백도 모두 같은 원리를 다르게 적용한 것입니다.
...
| Namespace | 환경·팀별로 리소스를 나누는 격리 단위 |
