...
컨테이너로 서비스를 하나 띄우는 건 어렵지 않습니다. 그런데 그 컨테이너가 수십 개가 되고, 죽으면 다시 살아나야 하고, 무중단으로 새 버전을 올려야 하고, 두 데이터센터에 나눠 떠 있어야 한다면 이야기가 달라집니다. 이 "여러 컨테이너를 대신 운영해 주는 도구"가 **쿠버네티스**입니다. 이 글은 쿠버네티스의 기본 개념부터 시작해, 제가 참여한 프로젝트에서 이를 **어떻게 실제 배포·운영에 적용했는지**까지 정리합니다. (클러스터가 내부적으로 어떤 구성 요소로 이루어지는지는 [쿠버네티스 구성 요소](/blog/kubernetes-components-architecture) 글에서 따로 정리했습니다.)
...
- **Service(서비스)** — 파드는 죽고 살아나며 IP가 계속 바뀌므로, 그 앞에 **고정된 진입점**을 두고 트래픽을 여러 파드에 **부하 분산**합니다. 외부에서 들어오는 경로는 **Ingress**가 담당합니다.
...
쿠버네티스는 강력하지만, **직접 운영하기는 까다롭습니다.** 클러스터의 제어를 맡는 **컨트롤 플레인**(API 서버·스케줄러·etcd 등)을 직접 설치·이중화·업그레이드·백업해야 하기 때문입니다. 프로젝트에서는 이 부담을 클라우드에 위임하는 **매니지드 쿠버네티스(AWS의 EKS)** 를 선택했습니다.
