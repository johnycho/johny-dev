...
포인트는 `sendOffsetsToTransaction` — <mark>오프셋 커밋을 트랜잭션에 넣어야 "처리 결과"와 "읽은 위치"가 한 몸이 됩니다.</mark> 그래서 컨슈머는 자동 커밋을 꺼야 합니다(`enable.auto.commit=false`).
...
분산 환경에선 "죽은 줄 알았던 옛 프로듀서(좀비)"가 되살아나 중복을 쓸 수 있습니다. `transactional.id`는 트랜잭션을 켜는 것 외에 **이 문제를 막는 열쇠**입니다.
