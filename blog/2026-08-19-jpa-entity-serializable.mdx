...
엔티티 클래스에 습관처럼 `implements Serializable`을 붙이는 경우가 많습니다. IDE가 권하기도 하고, "세션에 저장하려면", "캐시하려면", "API로 내보내려면" 같은 이유로요. 그런데 <mark>대부분의 경우 엔티티에 `Serializable`은 필요 없고, 오히려 지연 로딩·객체 그래프·버전 관리에서 문제를 유발합니다.</mark> 왜 붙이게 되는지(오해), 붙이면 무엇이 문제인지, 그리고 어떻게 해야 하는지 정리합니다. (영속성 컨텍스트 이야기는 [@DataJpaTest 컨텍스트 OOM](./spring-datajpatest-context-oom)도 참고)
...
- **HttpSession에 저장하려고** — 세션 클러스터링(세션을 디스크·다른 서버로 복제)하면 자바 직렬화가 일어나긴 합니다. 하지만 엔티티를 세션에 저장하는 것 자체가 권장되지 않습니다.
...
<mark>세션·캐시에 저장되는 시점은 대개 트랜잭션 밖이라, 지연 로딩 상태의 엔티티 직렬화는 문제가 발생하기 쉽습니다.</mark>

### 2. 연관관계를 타고 객체 그래프 전체가 함께 직렬화된다

자바 직렬화는 **참조로 도달 가능한 모든 객체**를 통째로 직렬화합니다. 엔티티는 양방향 연관·컬렉션으로 얽혀 있어, 하나를 직렬화하려다 연결된 그래프가 함께 포함됩니다.
...
```plantuml
@startuml
object "User 직렬화 시작" as U
object "orders" as O
object "orderItems" as I
object "product" as P
object "category → ..." as C
object "team → members → ..." as T
...
![PlantUML User 엔티티에서 연관을 타고 딸려오는 객체 그래프](/img/c4/pu-entity-graph.svg)

<mark>User 하나 저장하려다 주문·상품·팀까지, 사실상 DB의 절반이 세션·캐시로 함께 직렬화될 수 있습니다.</mark> `transient`로 끊을 수 있지만, 연관마다 챙겨야 해 관리 부담이 큽니다.
...
<mark>자주 바뀌는 엔티티를 직렬화 저장하면 배포 사이에 버전이 맞지 않아 깨지기 쉽습니다.</mark>

### 4. detached 스냅샷을 "엔티티처럼" 착각한다

역직렬화된 엔티티는 영속성 컨텍스트와 분리된 **과거 스냅샷**입니다. 다시 `merge`로 붙이지 않으면 변경 감지·지연 로딩이 전혀 안 되는 그냥 평범한 데이터인데, 생김새가 엔티티라 "관리되는 엔티티"로 착각하기 쉽습니다.

## 평소엔 멀쩡하다가 갑자기 터지는 상황

먼저 핵심 하나 — **세션에 저장한 객체는 "그 서버 밖으로 나갈 때"만 직렬화(바이트로 변환)됩니다.** 서버 안 메모리에 있는 동안엔 그냥 자바 객체라, 직렬화가 일어나지 않습니다. 그래서 문제가 이런 순서로 숨어 있습니다.

1. 로그인할 때 편의로 엔티티를 세션에 저장해 둡니다 — `session.setAttribute("loginUser", user)`. (`User`엔 `Serializable`, `orders`는 `LAZY`)
...
지금까지는 자바 직렬화(`Serializable`) 이야기였습니다. 그런데 컨트롤러가 `return user;`처럼 **엔티티를 그대로 응답으로 내보내는 것**도 비슷한 문제가 있는데, 이건 `Serializable`이 아니라 **JSON 직렬화(Jackson)** 에서 나는 **별개의** 문제입니다(Jackson은 `Serializable`을 보지 않습니다).
...
<mark>즉 `Serializable`을 붙였는지와 무관하게, 엔티티를 응답으로 내보내는 것 자체가 LAZY·그래프라는 같은 뿌리의 문제를 유발합니다.</mark> 그래서 해법도 같습니다 — DTO로 분리.

> **Q. `fetch join`·`@EntityGraph`로 미리 로딩하면 되지 않나?**
>
> **A.** 부분적으로만 맞습니다. 미리 초기화한 연관은 `LazyInitializationException`을 피하지만, ① **그래프에 안 넣은 다른 `LAZY` 연관**은 여전히 프록시라 문제가 발생할 수 있고, ② 이건 **쿼리마다** 지정해야 해 하필 "어쩌다 호출되는 엔드포인트"에서 빠뜨리기 쉽습니다. 또한 자바 직렬화(세션·캐시)에서는 미리 초기화해봐야 **초기화된 그래프가 오히려 더 크게 딸려가고**, `serialVersionUID`·detached 문제는 그대로입니다. 미리 로딩은 증상 하나를 가릴 뿐, 경계를 넘기는 문제는 DTO로 끊어야 합니다.

## 그럼 어떻게 — DTO로 분리

계층 밖(응답·세션·캐시)으로 나갈 데이터는 **엔티티 대신 DTO**로 옮겨, 필요한 필드만 명시적으로 채웁니다.
...
- **캐시** — 엔티티 대신 필요한 필드만 포함한 DTO/값 객체(직렬화 범위·버전이 명확).

<mark>DTO로 경계를 명확히 하면 지연 로딩·그래프 폭발·버전 문제를 원천적으로 피하면서, 나갈 데이터를 의도대로 통제할 수 있습니다.</mark>

## 정리

- 엔티티에 `Serializable`은 **대부분 불필요** — 특히 JSON 응답 목적이면 아무 상관이 없습니다.
- 붙이면 **지연 로딩 프록시 예외·그래프 전체 직렬화·`serialVersionUID` 버전 깨짐·detached 혼동**을 유발합니다.
