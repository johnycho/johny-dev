...
@startuml
start
:호출마다 새 OkHttpClient 생성;
:매번 새 Dispatcher(스레드 풀) 생성;
:retry로 실패 호출 증폭;
:스레드가 회수 안 되고 누적;
:몇 시간 뒤 네이티브 스레드 한도 도달;
:unable to create native thread → OOMKilled;
stop
@enduml
```
...
![PlantUML OkHttpClient 남발로 인한 스레드 누적 OOM](/img/c4/pu-oom-thread.svg)

<mark>트래픽이 그대로여도, 스레드가 조금씩 누적되면 몇 시간에 걸쳐 한도에 부딪힙니다 — 그래서 배포 직후가 아니라 반나절쯤 지나 발생한 것</mark>입니다.
...
- <mark>배포 직후가 아니라 몇 시간 뒤 발생하는 건 "천천히 쌓이는" 누수의 전형입니다.</mark>

느린 외부 호출이 유발하는 다른 장애는 [외부 API 하나가 느려지자 서버 전체가 멈췄다](./external-api-timeout-server-hang)도 함께 보세요.
