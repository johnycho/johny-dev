...
**핵심 원칙**: <mark>의존성은 항상 외부(어댑터) → 안(유즈케이스) 방향으로만 향합니다.</mark> 그래서 DB를 MySQL에서 MongoDB로 바꿔도, HTTP를 gRPC로 바꿔도 유즈케이스 코드는 그대로 유지됩니다.
...
**포트 = 주방의 출입구**
- 주방에는 출입구가 딱 2개 있음.
- **주문 받는 출입구(인바운드 포트)**: "이런 주문을 넣을 수 있어요"라는 메뉴판
- **재료 요청 출입구(아웃바운드 포트)**: "나는 이런 재료가 필요해요"라는 주문서

**어댑터 = 출입구로 드나드는 사람**
- **웨이터(인바운드 어댑터)**: 손님 주문을 받아서 → 주방 출입구에 넣어줌
- **식자재 납품업체(아웃바운드 어댑터)**: 주방이 요청한 재료 출입구에 → 실제 재료를 갖다줌
...
flowchart LR
    ...
    subgraph 안["안 (주방 = 코어)"]
        direction LR
        인포트["주문 출입구<br/>인바운드 포트"] --> 요리사(["요리사<br/>유즈케이스"])
        요리사 --> 아웃포트["재료 출입구<br/>아웃바운드 포트"]
    end
...
1. **포트는 "출입구/약속", 어댑터는 "그 출입구로 드나드는 실제 물건"**
   - 포트 = `interface` (약속만 정의)
...
<ArchifyEmbed src="/diagrams/hexagonal-ports-adapters.html" title="헥사고날 포트·어댑터 구조와 의존 방향 (인터랙티브)" height="460px" />
...
<mark>인바운드 포트는 그 자체가 **"이 어플리케이션이 할 수 있는 일(=유즈케이스)"의 목록**이라서, `~Port`보다 `~UseCase`라고 부르는 게 의미가 더 잘 드러나기 때문입니다.</mark> **둘 다 맞는 이름**인데, 관례적으로 인바운드는 `UseCase`를 씁니다.

### 왜 그런지

포트는 "코어가 외부 세계와 통신하는 출입구"인데, 인바운드와 아웃바운드는 출입구의 **성격이 다릅니다.**

**인바운드 포트 = "우리가 제공하는 기능 메뉴"**
- 외부에서 "너 뭐 할 수 있어?"라고 물었을 때 내미는 메뉴판
- 그 메뉴 한 줄 한 줄이 바로 **유즈케이스**(회원가입, 주문생성…)
...
이 규칙 하나를 지키기 위해 포트(interface)를 경계에 두고, 어댑터가 인바운드에서는 호출하고 아웃바운드에서는 구현하는 형태로 역할이 뒤집힙니다. 그 덕분에 웹 프레임워크나 DB 같은 "기술"이 바뀌어도 비즈니스 로직(유즈케이스)은 흔들리지 않고, 테스트도 포트만 교체하면 쉽게 할 수 있게 됩니다.
