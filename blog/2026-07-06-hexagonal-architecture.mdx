...
핵심은 **"안(도메인/코어)"과 "밖(외부 세계)"의 경계**입니다. 유즈케이스는 안쪽, 어댑터는 바깥쪽입니다.
...
```
손님          웨이터         [주문창구]    주방      [재료창구]      납품업체
       ─주문─▶ (인바운드    ─▶(인바운드 ─▶(요리사)─▶(아웃바운드─▶ (아웃바운드
              어댑터)         포트)                    포트)          어댑터)

              └─ 밖(바깥세계) ─┘    └──── 안(주방/코어) ────┘    └─ 밖(바깥세계) ─┘
```
...
인바운드 포트는 그 자체가 **"이 애플리케이션이 할 수 있는 일(=유즈케이스)"의 목록**이라서, `~Port`보다 `~UseCase`라고 부르는 게 의미가 더 잘 드러나기 때문입니다. **둘 다 맞는 이름**인데, 관례적으로 인바운드는 `UseCase`를 씁니다.
