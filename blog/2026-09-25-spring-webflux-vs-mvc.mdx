...
WebFlux는 이 문제를 **논블로킹**으로 해결합니다. 핵심 개념부터 살펴봅니다.
...
<mark>`Mono`·`Flux`는 "값" 자체가 아니라 **"나중에 값이 전달되는 자리"** 입니다 — 구독(subscribe)해야 실행되고, 결과는 콜백·연산자로 연결합니다.</mark>

### Reactive Streams — 네 가지 역할

WebFlux의 바탕인 **Reactive Streams**는 인터페이스 네 개로 구성됩니다. 이 관계만 이해하면 "왜 구독해야 실행되는지"·"백프레셔가 무엇인지"가 분명해집니다.

- **Publisher**(발행자) — 데이터를 만들어 내보내는 쪽. `Mono`·`Flux`가 여기에 해당합니다.
- **Subscriber**(구독자) — 데이터를 받는 쪽. `onNext`(값)·`onComplete`(끝)·`onError`(오류) 콜백을 가집니다.
- **Subscription**(구독) — Publisher와 Subscriber의 연결. 구독자가 `request(n)`으로 **받을 개수를 요청**하고, `cancel()`로 취소합니다.
- **Processor** — Publisher이자 Subscriber인 중간 단계(직접 쓸 일은 많지 않습니다).

네 역할이 실제로 어떻게 함께 동작하는지는 기본 인터페이스 코드로 한 번만 보면 이해됩니다(평소엔 연산자가 이 과정을 대신 처리합니다).
...
<mark>핵심은 **구독자가 `request(n)`으로 "처리할 수 있는 만큼만" 요청해 받는다**는 점입니다 — 이 pull 기반 요청이 백프레셔입니다. 발행자가 일방적으로 보내지 않습니다.</mark> (표준 자체·JDK Flow·cold/hot·라이브러리 관계는 [Reactive Streams 표준](./reactive-streams-spec)에서 자세히 다룹니다.)
...
### 연산자(operator) — 흐름을 구성한다

값을 직접 다루는 대신 **연산자로 단계를 연결합니다.** 자주 쓰는 것만:

- **`map`** — 값을 변환(1:1). `Mono<User> → Mono<UserDto>`.
- **`flatMap`** — 값을 **또 다른 Publisher로** 바꿔 연결(비동기 연쇄). `Mono<User> → Mono<Order>`.
...
- **스케줄러**: "어느 스레드에서 실행할지" 지정. `subscribeOn`(구독 시작 스레드)·`publishOn`(이후 단계 스레드). 불가피한 블로킹 작업은 `Schedulers.boundedElastic()`로 **이벤트 루프와 분리된 전용 풀에서** 실행한다.
  ```java
...
**MVC**: 요청을 받은 스레드가 I/O가 끝날 때까지 그 요청을 점유한 채 대기합니다.
...
<mark>MVC는 값을 바로 반환하지만, WebFlux는 `Mono`/`Flux`를 반환하고 `flatMap`·`map` 같은 **연산자로 흐름을 구성**합니다 — 코드의 사고방식 자체가 달라집니다.</mark>
...
- **중단점 디버깅이 잘 안 맞는다**: 선언(구성) 시점과 실제 실행(구독) 시점이 달라, 연산자 람다에 브레이크포인트를 걸어도 흐름을 따라가기 어렵습니다.
...
- **Reactor 디버그 모드 / `checkpoint()`**: 예외가 나도 스택에 내 코드가 안 보이므로, 요소마다 표식을 남긴다.
  ```java
  // (개발) 전역 디버그 — 구성 지점을 스택에 복원, 대신 비용이 큼
  ...
  paymentClient.charge(order)
               .checkpoint("결제 조회 단계")
               .flatMap(this::save);
  // 더 가벼운 대안: reactor-tools의 ReactorDebugAgent.init() (오버헤드 없이 구성 지점 복원)
  ```
...
- **적은 스레드로 높은 동시성**: I/O 대기가 많은 서비스(게이트웨이·조합 API)에서 스레드 풀 고갈 없이 많은 요청을 처리합니다.
- **백프레셔**: 생산자-소비자 속도 차를 규격 차원에서 제어합니다 — 예: 느린 클라이언트에게 소비 속도보다 빠르게 보내지 않습니다.
