...
<mark>확장은 한 번에 끝내는 작업이 아니라, 병목이 나타나는 지점을 측정해 그 병목만 푸는 반복 과정입니다.</mark> 아직 오지 않은 부하를 미리 가정해 복잡하게 만들면 유지보수 비용만 늘어납니다. 각 단계는 복잡도라는 비용을 더하므로, **지표로 한계를 확인한 뒤** 다음 단계로 넘어갑니다. 트래픽이 커질 때 실제로 나타나는 증상은 [트래픽 증가가 유발하는 문제들](./traffic-growth-problems)에서 정리했습니다.
...
Person(client, "클라이언트")
Container(api, "API 서버")
ContainerDb(db, "데이터베이스")
...
![C4 단일 구조: 클라이언트–API 서버–데이터베이스](/img/c4/scaling-1.svg)

장점은 단순함입니다. 배포·디버깅·트랜잭션이 모두 쉽습니다. 한계는 두 가지입니다. 트래픽이 커지면 한 대의 CPU·메모리·디스크가 곧 상한에 닿고, 그 한 대가 멈추면 서비스 전체가 멈춥니다(단일 장애점, SPOF: Single Point Of Failure). <mark>단순 구조의 진짜 약점은 성능보다 "한 곳이 멈추면 전부 멈춘다"는 점입니다.</mark>
...
**언제**: 이벤트 오픈으로 동시 접속이 평소의 10배가 됩니다. 한 대로는 요청이 쌓여 응답이 밀립니다. 또한 로그인 세션을 서버 메모리에 두었더니, 인스턴스를 늘리자 다음 요청이 다른 서버로 가서 "로그인이 자꾸 풀리는" 문제가 생깁니다.
...
Person(client, "클라이언트")
Container(lb, "로드밸런서")
Container(api1, "API 인스턴스 1")
Container(api2, "API 인스턴스 2")
Container(api3, "API 인스턴스 3")
ContainerDb(db, "DB")
ContainerDb(redis, "Redis 세션 저장소")
...
Rel(api2, db, "읽기·쓰기")
Rel(api3, db, "읽기·쓰기")
Rel(api1, redis, "세션·상태")
Rel(api2, redis, "세션·상태")
Rel(api3, redis, "세션·상태")
@enduml
```

</details>

![C4 로드밸런서로 분산되는 다중 API 인스턴스와 공유 Redis 세션 저장소](/img/c4/scaling-2.svg)
...
정합성 관점의 과제는 **로컬 상태 불일치**입니다. 인스턴스마다 들고 있는 메모리 캐시·카운터가 다르면, 같은 사용자가 요청할 때마다 다른 값을 볼 수 있습니다. 해결의 방향은 하나입니다 — 여러 인스턴스가 공유해야 하는 상태는 외부 저장소로 분리합니다. <mark>수평 확장의 전제 조건은 무상태입니다. 상태를 서버 밖(외부 저장소)으로 분리해야 인스턴스를 자유롭게 늘릴 수 있습니다.</mark>
...
ContainerDb(r1, "Replica 1")
ContainerDb(r2, "Replica 2")
...
![C4 쓰기는 Primary, 읽기는 Replica로 분리하고 Primary가 Replica로 복제](/img/c4/scaling-3.svg)
...
핵심은 락을 넓게 오래 잡지 않는 것입니다. <mark>동시성 문제는 "동시에 같은 것을 건드릴 때"만 생깁니다 — 경합 범위를 줄이고, 조건부 연산·낙관적 락으로 잠그는 구간을 최소화합니다.</mark>
...
![C4 API가 메시지 큐로 이벤트를 발행하고 컨슈머들이 소비해 DB에 기록](/img/c4/scaling-4.svg)
...
Container(api, "API 서버")
ContainerDb(db, "DB", "업무 테이블 + Outbox 테이블")
Container(relay, "Outbox 릴레이")
ContainerQueue(queue, "메시지 큐")

Rel(api, db, "1 같은 트랜잭션")
Rel(relay, db, "2 Outbox 폴링")
Rel(relay, queue, "3 발행")
@enduml
```

</details>

![C4 Outbox 패턴: 같은 트랜잭션으로 DB에 기록 후 릴레이가 폴링해 큐로 발행](/img/c4/scaling-5.svg)
...
데이터를 나눕니다. 먼저 한 DB 안에서 테이블을 나누는 **파티셔닝**, 그다음 여러 DB로 분산하는 **샤딩**입니다.
...
![C4 샤드 라우터가 해시로 요청을 여러 샤드에 분산](/img/c4/scaling-6.svg)
...
핵심 원칙은 하나입니다. **측정으로 병목을 확인하고, 그 병목만 한 단계 확장합니다.** 단계가 올라갈수록 강한 일관성(단일 트랜잭션)에서 최종 일관성으로, 단순함에서 복잡함으로 이동합니다. 그래서 필요할 때까지는 앞 단계에 머무는 것이 좋은 설계입니다.
